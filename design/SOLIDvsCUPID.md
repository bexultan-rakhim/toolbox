...
***For automation, a system that fails gracefully is worth more than a system that is easy to extend.***

Today's topic is CUPID - a set of properties for good code proposed by [Dan North](https://dannorth.net/blog/cupid-for-joyful-coding/) - and why it fits real-world engineering, especially in robotics, better than the more famous SOLID principles. This is not about dismissing SOLID entirely. It is about recognizing that SOLID was born from enterprise software, not from systems where a bug can stop a factory line or crash a robot arm.
...
Now your robot fails at 3am on the production floor. You look at the stack trace. You see:

```
MotorController.execute() -> BaseController.execute() -> AbstractActuator.dispatch() -> ???
```
...
LSP is the most academically stated of the five principles, and I bet no one actually applies it correctly. It is quite theorietical stuff. 
...
I have seen this happen in real system, and Believe it or not, the names were roughly the same, too.
...
You have replaced one concrete type with three abstract ones. Have you made the code more understandable? Almost certainly not. You have created **abstractions for their own sake** - the thing CUPID is designed to resist. The interface graph becomes its own maintenance burden. New engineers spend time navigating interface definitions rather than understanding what the system does.
...
- **I**diomatic - feels natural in its language
- **D**omain-based - uses the language of the problem domain

The most important for a robotics engineer is **Predictability**. Predictability means: I can read this code and know what it will do. I can look at a failure and trace directly to its cause. I do not need to reconstruct a runtime call graph or an inheritance tree in my head.
...
SOLID was designed in a world dominated by Java and C++, where polymorphism was the primary tool for managing complexity. Two languages push back on that assumption structurally:

**C** - there are many arguments that you can use C++ for embedded. I think this is only partially true. There is a reason why C still dominates embedded world and I bet it is also because you can not bring SOLID to C. It's syntax simply resists the world of Enterprise and there is a reason why business logic is written mostly in Java, and Linux written in C. C has no inheritance, no virtual dispatch. Unix was written in C. Error handling is explicit returns from functions. 

That said, the honest counterargument is that C gives you enough rope to hang yourself in other ways — pointer arithmetic, manual memory management, no type safety on void pointers. So the predictability argument cuts both ways: yes, the control flow is transparent, but memory bugs can be deeply unpredictable and hard to trace. 

**Go** has no inheritance at all. No `extends`, no virtual methods, no classes. Composition through struct embedding, interfaces satisfied implicitly. The language cannot build a six-layer inheritance hierarchy because it does not provide the ladder. Errors are explicit return values - the caller deals with failure at the call site, in the same file you are reading. That said, **Go** is garbage collected language. There is that.

**Rust** makes enums and `match` first-class. Exhaustive pattern matching turns the switch statement from a code smell into a compiler-enforced contract. The type system encodes domain variants directly. Fortunately, Rust has `unsafe`, when compiler works against you.

So, these languages push you toward code that is flat, explicit, and domain-shaped - which is exactly what CUPID asks for. 
...
Enterprise software often inverts this. Flexibility and extensibility are prized because business requirements shift rapidly and teams are large. Subtile failes are not often big deal, because often it is a bit of inconvinience, than say danger to human life (Don't get me wrong, I think portions of the enterprise software that does something really important often is not designed by SOLID principle). SOLID was designed for that world. But a robot on a factory floor has a finite and well-understood set of sensors, actuators, and operating modes. These are **closed domains**. The cost of a production outage far exceeds the cost of touching a `switch` statement.

SOLID asks: *How do I extend this system without touching existing code?*

CUPID asks: *How do I make this system obvious, so that when something goes wrong, anyone on the team can find the problem in minutes?*
...
Every layer of abstraction is a layer of indirection. Prefer structures you can read in one file over hierarchies that require tracing through multiple classes. If you are writing `super()` calls that chain three levels up, that is a signal to flatten.

But also, related to this - **keep interfaces narrower than underlying logic**. Here, flatness is about number of abstraction layers, not having flat interface (i.e. have single class with hundred of functions). 

### 2. Use Enums and Match/Switch for Closed Domains

When your domain has a fixed set of known variants - sensor types, motor states, error codes, command types - express that with an enum and handle it with a switch or match. The explicitness is a feature. The exhaustiveness check is a safety net the compiler provides for free.

Similarly, you can use [dispatch table](https://blog.alicegoldfuss.com/function-dispatch-tables/) instead of polymorphism. 

### 3. Make Failure Visible

Prefer early returns (although not good idea in **C**, specifically) and explicit error values over exceptions that unwind through polymorphic call stacks. A function that returns `(value, error)` forces the caller to confront the failure at the call site. An exception from deep inside an inheritance hierarchy forces you to reconstruct the full context of four layers before you can understand what failed.

### 4. Name Things After the Domain

CUPID's Domain-based property is underrated. Name your structs, functions, and modules after what they represent in the physical world - `MotorCommand`, `JointAngle`, `SafetyZone`. Code that maps directly to the problem domain is faster to read under pressure than code that maps to a design pattern.

### 5. Earn Every Abstraction

Before introducing an interface or base class, ask: does this abstraction reflect something real in the domain, or does it exist to satisfy a principle? If the answer is the latter, delete it. The best abstraction is often no abstraction. The second best is one that can be deleted in an afternoon without a four-hour refactor.
